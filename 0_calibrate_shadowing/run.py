#!/usr/bin/env python3
"""
STEP 0 -- CALIBRATE BODY SHADOWING  (do this BEFORE you set SHADOW = True)
=========================================================================
...
INPUTS
  ...
  BODY_DIR/body_profile.csv    from step 2a/2b
...
DATASETS_DIR = os.path.join("..", "3a_doors", "Datasets")
BODY_DIR = os.path.join("..", "2a_solve_body_local", "Body")
# ────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    prof = os.path.join(_here(BODY_DIR), "body_profile.csv")
    if not os.path.exists(prof):
        raise SystemExit(f"no {prof} -- run 2a_solve_body_local (or 2b) first.")
    try:
        verify_body_artifact_bundle(_here(BODY_DIR))
    except ValueError as exc:
        raise SystemExit(f"uncommitted or changed body bundle: {exc}") from exc
    gen = np.loadtxt(prof, delimiter=",", skiprows=1)
